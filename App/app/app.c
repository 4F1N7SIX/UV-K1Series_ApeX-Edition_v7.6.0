...
Skip:
    switch (Mode)
    {
        ...
        case END_OF_RX_MODE_END:
            /*
             * Complete receiver shutdown after MDC packet reception ends.
             * The stale squelch flag must be cleared, otherwise the radio still
             * thinks an RF signal is present and re-enters the incoming/monitor
             * receive path on the next loop, which re-enables audio and hiss.
             */
            BK4819_ResetFSK();
            gMdcRxIndex = 0;

            // Clear the stale squelch/open-RF state that triggers the re-entry path.
            g_SquelchLost = false;
            gMonitor      = false;
            UI_MAIN_SetRxLed(false);

            // Fully disable the FSK RX interrupt path so the next scheduler tick
            // cannot immediately re-enter the receive state and restore monitor hiss.
            BK4819_WriteRegister(BK4819_REG_3F, 0x0000);
            BK4819_Idle();

            // Mute audio at the chip level
            BK4819_SetAF(BK4819_AF_MUTE);

            // Disable audio hardware path completely
            AUDIO_AudioPathOff();
            gEnableSpeaker = false;

            // Force power-save idle instead of active RX
            // This prevents monitor noise from being heard after packet ends
...
void APP_StartListening(FUNCTION_Type_t function)
{
    ...
    // Enable MDC RX FSK mode to capture incoming MDC frames
    if (function == FUNCTION_RECEIVE)
    {
        BK4819_PrepareFSKReceive();
        gMdcRxIndex = 0;  // reset MDC buffer
    }
...
//          g_CTCSS_Lost = true;
        ...
        {
            bool should_process_mdc = true;

            #ifdef ENABLE_AIRCOPY
                if (gScreenToDisplay == DISPLAY_AIRCOPY &&
                    gAircopyState == AIRCOPY_TRANSFER &&
                    gAirCopyIsSendMode == 0)
                    should_process_mdc = false;
            #endif
            ...
            if (should_process_mdc) {
                const unsigned int wordsToRead = interrupts.fskRxFinied ? (MDC1200_FIFO_WORD_COUNT - gMdcRxIndex) : 4;
                for (unsigned int i = 0; i < wordsToRead; i++) {
                    const uint16_t word = BK4819_ReadRegister(BK4819_REG_5F);
                    if (gMdcRxIndex < MDC1200_FIFO_WORD_COUNT)
                        gMdcRxBuffer[gMdcRxIndex++] = word;
                }

                if (interrupts.fskRxFinied && gMdcRxIndex >= MDC1200_FIFO_WORD_COUNT) {
                    uint8_t op = 0;
                    uint8_t arg = 0;
                    uint16_t unit_id = 0;
                    bool valid = false;

                    if (MDC1200_DecodeFrameWords(gMdcRxBuffer, MDC1200_FIFO_WORD_COUNT, &op, &arg, &unit_id, &valid) == MDC1200_ERROR_NONE && valid) {
                        sprintf(gMdcDecodedId, "%04X", unit_id);
                        gMdcLastUnitId = unit_id;
                        gMdcDecodeCountdown_500ms = 6000u / 500u;
                        gUpdateDisplay = true;
                    }
                    gMdcRxIndex = 0;
                }
...
void APP_TimeSlice500ms(void)
{
    ...
    if (gMdcDecodeCountdown_500ms > 0)
    {
        if (--gMdcDecodeCountdown_500ms == 0)
        {
            gMdcDecodedId[0] = '\0';
